...
First, users can download the latest package through github through the following code 'devtools::install_github("R-4-Data-Science/Final-Project-Group-11")'.(If the user does not have the ‘devtools’ package installed, this package needs to be installed first. The MASS package also needs to be installed and loaded.)
...
library(MASS)
```
Next, we will use the 'bank' data set from the canvas page to display the functions in our package. For this example, let's complete a logistic regression for Defaulting based on age and balance. We will have to change the `default` column so that a 0 means the person did not default, and a 1 means the person did default
```{r import example data set, include=TRUE}
  bank <- read.csv("C:/Users/charl/Downloads/bank.csv", sep=";")

for(i in 1:nrow(bank)){
  if(bank[i,5] == 'no'){bank[i,5] <- 0}
  else bank[i,5] <- 1
}
bank[,5] <- as.integer(bank[,5])
```
...
test_resp <- 5
test_pred <- c(1,6)
log_betas(Data = bank, Y = test_resp, X = test_pred)
```
...
results <- suppressWarnings(bootstrap_logistic_confidence(bank, test_resp, test_pred))
print(results)
...
logistic_plot(bank, Resp = 5, Pred= 6)
...
result <- confusion_matrix(Data = bank, X=test_pred, y=test_resp, cut_off = 0.5)
print(result)
...
plot_confusion_matrix(Data = bank, y = test_resp,X = test_pred , value_name = 'false_discovery_rate')
# The example for not exist.
plot_confusion_matrix(Data = bank, y = test_resp,X = test_pred , value_name = 'false_discovery')
```
